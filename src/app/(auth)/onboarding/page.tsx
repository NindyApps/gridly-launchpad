...
const DATA_SOURCES = [
  ...
  { id: "hackernews", name: "Hacker News", desc: "Monitor tech discussions & signals" },
              ...
              {DATA_SOURCES.map((src) => (
                <button key={src.id} onClick={() => toggleSource(src.id)}
                  className={`text-left p-4 rounded-lg border-2 transition-colors ${
                    selectedSources.includes(src.id) ? "border-accent bg-accent/5" : "border-border hover:border-muted-foreground/30"
                  }`}>
              ...
              {CRM_OPTIONS.map((crm) => (
                <button key={crm.id} onClick={() => setSelectedCrm(crm.id)}
                  className={`p-4 rounded-lg border-2 transition-colors text-center ${
                    selectedCrm === crm.id ? "border-accent bg-accent/5" : "border-border hover:border-muted-foreground/30"
                  }`}>
