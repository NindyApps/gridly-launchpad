// AES-GCM token encryption for Next.js (Node.js 18+ / Edge Runtime).
...
// Set as ENCRYPTION_KEY in your Replit Secrets / environment variables.
// Also add to Supabase Edge Function Secrets under the same name.

function getKeyData(): Uint8Array {
  const key = process.env.ENCRYPTION_KEY
  if (!key) throw new Error('[encryption] ENCRYPTION_KEY environment variable is not set')
  if (key.length !== 32) {
    throw new Error(
